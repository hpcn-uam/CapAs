...
#include "../../include/hpcap.h"
	...
	#include "../hpcap_ixgbe-3.7.17_buffer/driver/ixgbe_type.h"
...
)
{
	...
	while( cnt < limit )
	{
		...
		do
		{
			...
			if( jf == 0 )
			{
				pkt_info = le16_to_cpu(rx_desc->wb.lower.lo_dword.hs_rss.pkt_info);
				hdr_info = le16_to_cpu(rx_desc->wb.lower.lo_dword.hs_rss.hdr_info);
			}
		...
		/******************************************
		 Packet format in the RAW stream:
		   ... | Seconds 32b | Nanosec 32b | Caplen 16b | Length 16b | Hdr_info 16b | Pkt_info 16b | Hash RSS 32b | ... data ... |
		 NOTE:
		...
		******************************************/
		...
		if( unlikely( padding ) )
		{
			...
			*((u16 *)&tmp_h[2*sizeof(u32)+2*sizeof(u16)]) = 0; // hdr_info
			*((u16 *)&tmp_h[2*sizeof(u32)+3*sizeof(u16)]) = 0; // pkt_info
			*((u32 *)&tmp_h[2*sizeof(u32)+4*sizeof(u16)]) = 0; // RSS HASH
		...
		*((u16 *)&tmp_h[2*sizeof(u32)+2*sizeof(u16)]) = ( (hdr_info >> 5) & 0x03FF );
		*((u16 *)&tmp_h[2*sizeof(u32)+3*sizeof(u16)]) = pkt_info;
		*((u32 *)&tmp_h[2*sizeof(u32)+4*sizeof(u16)]) = rss_hash;
		if( (offset + RAW_HLEN) > bufsize )
		{
			aux = bufsize-offset;
			memcpy(pkt_buf+offset, tmp_h, aux);
