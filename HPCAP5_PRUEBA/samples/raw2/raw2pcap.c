...
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		#endif
		{
			...
			if( fread(&hdr_info,1,sizeof(u_int16_t),fraw)!=sizeof(u_int16_t) )
			{
				printf("hdr_info\n");
				break;
			}
			if( fread(&pkt_info,1,sizeof(u_int16_t),fraw)!=sizeof(u_int16_t) )
			{
				printf("pkt_info\n");
				break;
			}
			if( fread(&hash_rss,1,sizeof(u_int32_t),fraw)!=sizeof(u_int32_t) )
			{
				printf("hash_rss\n");
				break;
			}
