...
inline int ASCII_filter_saltos(void *header, u_char *pbuffer, int nbytes_filter,char *tabla_ascii){
...
busqContinua:
	if(j+len_ini_racha > nbytes_filter) goto finFiltroAsciiSaltos;
    for(i=0; i<len_ini_racha ; i++){
    	if( !tabla_ascii[pbuffer[j+i]] )//Byte i-esimo NO ASCII
    		goto busqSaltos;
...
busqSaltos:
	for(j=j+i+12 ; j<nbytes_filter ; j+=12){
		num_bytes_analizados++;
		if( tabla_ascii[pbuffer[j]] ){//Byte i-esimo ASCII
				legiblesASCII+=100;
				if( !tabla_ascii[pbuffer[j-1]] ){ 
					j=j-1;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-2]] ){ 
					j=j-2;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-3]] ){ 
					j=j-3;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-4]] ){ 
					j=j-4;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-5]] ){ 
					j=j-5;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-6]] ){ 
					j=j-6;i=0;
					goto busqSaltos;
				}else if( !tabla_ascii[pbuffer[j-7]] ){ //Los bytes j-6:j son ASCII busco racha continua pero de 7 mas a partir de j+1
					j=j+1;
					len_ini_racha=COEF_LEN_RACHA-7;
					goto busqContinua;
				}else if( !tabla_ascii[pbuffer[j-8]] ){ //Los bytes j-7:j son ASCII busco racha continua pero de 6 mas a partir de j+1
					j=j+1;
					len_ini_racha=COEF_LEN_RACHA-8;
					goto busqContinua;
				}else if( !tabla_ascii[pbuffer[j-9]] ){ //Los bytes j-9:j son ASCII busco racha continua pero de 5 mas a partir de j+1
					j=j+1;
					len_ini_racha=COEF_LEN_RACHA-9;
					goto busqContinua;
				}else if( !tabla_ascii[pbuffer[j-10]] ){ //Los bytes j-10:j son ASCII busco racha continua pero de 5 mas a partir de j+1
					j=j+1;
					len_ini_racha=COEF_LEN_RACHA-10;
					goto busqContinua;
				}else if( !tabla_ascii[pbuffer[j-11]] ){ //Los bytes j-11:j son ASCII busco racha continua pero de 4 mas a partir de j+1
					j=j+1;
					len_ini_racha=COEF_LEN_RACHA-11;
					goto busqContinua;
				}else //12 bytes anteriores a j son ASCII, racha encontrada
					return 1;
			
		}
	}
finFiltroAsciiSaltos:
...
inline int ASCII_filter(void *header, u_char *pbuffer, int nbytes_filter,char *tabla_ascii){
	struct raw_header *head = (struct raw_header *) header;
		...
		else
		{
			...
			else
			{
				*pbuffer = ((u_char*)handle->buf)+offs;
				//prefetch0( pbuffer );
				//prefetch0( pbuffer+64 );
				//prefetch0( pbuffer+128 );
				//prefetch0( pbuffer+192 );
				//prefetch0( pbuffer+256 );
			}
