...
int main(int argc, char **argv)
{
		...
		while( (!stop) && (i < HPCAP_FILESIZE) )
		{
			...
			if( hp.acks < hp.avail )
			{
				...
				{	
					...
					num_paquetes++;
					num_bytes_leidos+=phead_hpcap.caplen;
					if( ASCII_filter(&phead,bp, N_BYTES_ANALYSIS) ){
						//escribir completo
						num_bytes_escritos+=phead_hpcap.caplen;
						//write paquete al buffer (con padding)
					}else{
						//escribir solo la cabecera
						phead_hpcap.caplen = phead.hdr_info; //corto el paquete al tamaño de las cabeceras
						num_bytes_escritos+=phead_hpcap.caplen;
						//write paquete al buffer (con padding)
					}
					
					if (fd ){
						ret = write(fd, bp, phead_hpcap.caplen+RAW_HLEN);
					}
						
					i += phead_hpcap.caplen+RAW_HLEN;
				}
			}
		}
		hpcap_ack( &hp);
	...
	printf("\tPaquetes: %d\n",num_paquetes);
	printf("\tBytes escritos: %d / Bytes totales: %d\n",num_bytes_escritos,num_bytes_leidos);
